...
//FUNCIÓN PARA TRAER LOS USUARIOS GUARDADOS EN EL ARCHIVO LOCAL DE 

    fetch("../json/usersDB.json")
        .then((response) => response.json())
        .then(({ userDB }) => {

            const usuario = userDB.find((user) => user.username === userData && user.password === passwordData);

//CONDICIONAL PARA VALIDAR SI EL USUARIO Y CONTRASEÑA INGRESADO ESTÁ REGISTRADO
            
            if ((userData === checkUserInfo.user && passwordData === checkUserInfo.password) || usuario) {
                Toastify({
